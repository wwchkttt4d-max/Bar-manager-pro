...
(function() {
  ...
  if (!cont) return; // Sécurité : évite de bloquer le script si l'élément est absent
  const colors = ['#00d2c8','#a855f7','#f5c842','#ff9f43','#ff6b9d','#3b82f6'];
  for (let i = 0; i < 18; i++) {
    const p = document.createElement('div');
    p.className = 'particle';
    const size = Math.random() * 4 + 2;
    p.style.cssText = `inline-size:${size}px;block-size:${size}px;inset-inline-start:${Math.random()*100}%;` +
...
function demoTimestamp(date = new Date()) {
...
window._showLogin = function() {
  ...
  _unsubStock = _unsubNotif = _unsubActivity = _unsubVentes = null;
  const ls = document.getElementById('loginScreen');
  if (!ls) return;
  const appEl = document.getElementById('app');
...
async function tryLogin() {
  ...
  try {
    ...
    btn.disabled = false;
    document.getElementById('loginBtnText').style.display = 'inline'; // On ne vide plus le texte pour qu'il revienne en cas d'erreur
...
// Vérification de sécurité : si Firebase a déjà détecté l'utilisateur avant le chargement de app.js
if (window._currentFirebaseUser && window._showApp) {
  window._showApp(window._currentFirebaseUser);
}
...
function startDemoMode() {
  demoMode = true;
  window._gammaDemoMode = true;
  window._currentFirebaseUser = { email:'demo@gamma.local' };
  showToast('Mode démo activé — toutes les données restent locales', 'info');
  window._showApp(window._currentFirebaseUser);
}

if (new URLSearchParams(window.location.search).get('demo') === '1') {
  window.addEventListener('DOMContentLoaded', () => startDemoMode(), { once:true });
}
...
function setupDemoMode() {
  const saved = localStorage.getItem('gamma_demo_cache');
  if (saved) {
    try {
      const parsed = JSON.parse(saved);
      cache.stock = parsed.stock || [];
      cache.ventes = parsed.ventes || [];
      cache.notifications = parsed.notifications || [];
      cache.activity = parsed.activity || [];
      rehydrateDemoDates();
    } catch(e) {
      localStorage.removeItem('gamma_demo_cache');
    }
  }
...
function hydrateDemoData() {
  const now = new Date();
  cache.stock = SEED_STOCK.map((item, i) => {
    const factor = [0, 0.5, 1, 1.8, 2.4][i % 5];
    const qty = Math.max(0, Math.round((item.quantiteMin || 1) * factor));
    return {
      _id:`demo-stock-${i+1}`,
      ...item,
      quantite: qty,
      updatedAt: demoTimestamp(new Date(now.getTime() - i * 36e5)),
      updatedBy: 'demo@gamma.local'
    };
  });

  const products = cache.stock.filter(s => ['spiritueux','bieres','softs','sirops'].includes(s.categorie)).slice(0, 16);
  cache.ventes = products.map((p, i) => {
    const d = new Date(now);
    d.setDate(now.getDate() - (i % 7));
    d.setHours(17 + (i % 7), 15, 0, 0);
    const qty = (i % 4) + 1;
    return {
      _id:`demo-vente-${i+1}`,
      produit:p.nom,
      ...
      prix:p.prix || 8,
      total:qty * (p.prix || 8),
      date:demoTimestamp(d),
      user:i % 3 ? 'serveur@gamma.local' : 'manager@gamma.local'
    };
  });

  cache.notifications = [
    { _id:'demo-notif-1', type:'stock_critical', message:'🚨 Rupture détectée : Angostura Bitter', produit:'Angostura Bitter', createdAt:demoTimestamp(new Date(now.getTime() - 22 * 6e4)), readBy:[] },
    { _id:'demo-notif-2', type:'stock_low', message:'⚠️ Stock faible : Coca-Cola Zero 33cl btl', produit:'Coca-Cola Zero 33cl btl', createdAt:demoTimestamp(new Date(now.getTime() - 75 * 6e4)), readBy:[] },
    { _id:'demo-notif-3', type:'team_message', message:'💬 Manager : pensez à valider l’inventaire avant fermeture', produit:null, createdAt:demoTimestamp(new Date(now.getTime() - 130 * 6e4)), readBy:[] }
  ];

  cache.activity = [
    { _id:'demo-act-1', type:'stock_update', icon:'📉', description:'<strong>Serveur</strong> a retiré 2× <strong>Coca-Cola Zero 33cl btl</strong>', timestamp:demoTimestamp(new Date(now.getTime() - 18 * 6e4)) },
    { _id:'demo-act-2', type:'vente', icon:'💰', description:'<strong>Manager</strong> a vendu 3× <strong>Bombay Sapphire 70cl</strong>', timestamp:demoTimestamp(new Date(now.getTime() - 42 * 6e4)) },
    { _id:'demo-act-3', type:'order', icon:'📦', description:'<strong>Demo</strong> a préparé une commande fournisseur', timestamp:demoTimestamp(new Date(now.getTime() - 95 * 6e4)) }
  ];
}
...
function persistDemo() {
  ...
  const serialize = arr => arr.map(item => {
    const copy = { ...item };
    ['updatedAt','createdAt','timestamp','date'].forEach(k => {
      if (copy[k]?.toDate) copy[k] = copy[k].toDate().toISOString();
    });
    return copy;
  });
  localStorage.setItem('gamma_demo_cache', JSON.stringify({
    stock: serialize(cache.stock),
    ventes: serialize(cache.ventes),
    notifications: serialize(cache.notifications),
    activity: serialize(cache.activity)
  }));
}

function rehydrateDemoDates() {
  ['stock','ventes','notifications','activity'].forEach(list => {
    cache[list].forEach(item => {
      ['updatedAt','createdAt','timestamp','date'].forEach(k => {
        if (typeof item[k] === 'string') item[k] = demoTimestamp(item[k]);
      });
    });
  });
}
...
async function resetStock() {
  ...
  if (demoMode) {
    ...
    persistDemo();
    setupDemoMode();
    showToast(`✅ Démo réinitialisée — ${SEED_STOCK.length} produits chargés`, 'success');
    return;
  }
...
function updateDashboard() {
  ...
  const ca      = cache.ventes.reduce((s,v) => s + (v.total||0), 0);
  const tickets = cache.ventes.length;
  const reorderValue = cache.stock
    .filter(s => (s.quantite||0) <= (s.quantiteMin||0))
    .reduce((sum, s) => sum + Math.max(0, (s.quantiteMin||0)*2 - (s.quantite||0)) * (s.prix||0), 0);
  ...
  setEl('d-businessMode', demoMode ? 'Démo' : 'Live');
  ...
  if (alertEl) {
    ...
    alertEl.innerHTML = low.length
      ? low.map(s => `
        <div style="display:flex;justify-content:space-between;align-items:center;padding:0.5rem 0;border-block-end:1px solid var(--border)">
  ...
  const pilotEl = document.getElementById('d-businessPilot');
  if (pilotEl) {
    const avgTicket = tickets ? ca / tickets : 0;
    const riskLabel = alertes === 0 ? 'Stock maîtrisé' : alertes < 5 ? 'Surveillance légère' : 'Action prioritaire';
    const riskBadge = alertes === 0 ? 'badge-green' : alertes < 5 ? 'badge-amber' : 'badge-red';
    pilotEl.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0.8rem">
        <div style="padding:0.8rem;border:1px solid var(--border);border-radius:10px;background:rgba(255,255,255,0.03)">
          <div style="color:var(--muted);font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em">CA suivi</div>
          <div style="font-family:'Playfair Display',serif;font-size:1.45rem;color:var(--gold)">${fMoney(ca)}</div>
        </div>
        <div style="padding:0.8rem;border:1px solid var(--border);border-radius:10px;background:rgba(255,255,255,0.03)">
          <div style="color:var(--muted);font-size:0.72rem;text-transform:uppercase;letter-spacing:0.08em">Panier moyen</div>
          <div style="font-family:'Playfair Display',serif;font-size:1.45rem;color:var(--teal)">${fMoney(avgTicket)}</div>
        </div>
      </div>
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:0.9rem;padding-top:0.9rem;border-top:1px solid var(--border)">
        <span style="color:var(--muted);font-size:0.86rem">Risque opérationnel</span>
        <span class="badge ${riskBadge}">${riskLabel}</span>
      </div>
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:0.6rem">
        <span style="color:var(--muted);font-size:0.86rem">Budget réassort conseillé</span>
        <strong style="color:var(--text)">${fMoney(reorderValue)}</strong>
      </div>`;
  }
}
  ...
  grid.innerHTML = items.map(s => {
    ...
    <div class="gamma-card ${stCls}" data-id="${s._id}" data-nom="${esc(s.nom)}">
      <div class="gamma-card-cat">
        ${cat.emoji} ${cat.label}
        ${isCrit ? '<span class="badge badge-red" style="margin-inline-start:auto">Rupture</span>'
          : isLow ? '<span class="badge badge-amber" style="margin-inline-start:auto">⚠ Faible</span>' : ''}
      </div>
      ...
      <div class="gamma-qty-row">
        ...
        <div style="margin-inline-start:auto;font-size:0.75rem;color:var(--muted)">min: ${s.quantiteMin||0}</div>
      </div>
      <div class="gamma-progress">
        <div class="gamma-progress-bar" style="inline-size:${pct}%;background:${bar}"></div>
      </div>
      ...
      <div class="gamma-card-actions">
        ...
        <button class="btn btn-ghost btn-sm"  data-action="edit"   style="margin-inline-start:auto">✏️</button>
...
async function quickAddStock(id, delta) {
  ...
  if (demoMode) {
    const oldQty = item.quantite || 0;
    item.quantite = newQty;
    item.updatedAt = demoTimestamp();
    item.updatedBy = who();
    await logActivity('stock_update', delta>0?'📈':'📉',
      ...
      who());
    if (newQty <= (item.quantiteMin||0) && oldQty > (item.quantiteMin||0)) {
      await pushNotif(
...
function openAddProduitGamma() {
  openModal('📦','Nouveau Produit',`
    ...
    <button class="btn btn-teal" style="inline-size:100%;margin-block-start:0.5rem" onclick="saveNewProduit()">✓ Ajouter au stock</button>
  `);
}
...
async function saveNewProduit() {
  ...
  if (demoMode) {
    cache.stock.push({
      ...
      createdAt: demoTimestamp(), updatedAt: demoTimestamp(), updatedBy: who()
    });
    ...
    renderGammaStock();
    updateDashboard();
    showToast(`${nom} ajouté au stock démo`, 'success');
    return;
  }
...
function openEditStock(id) {
  ...
  openModal('✏️',`Modifier : ${s.nom}`,`
    ...
    <button class="btn btn-teal" style="inline-size:100%;margin-block-start:0.5rem"
...
async function saveEditStock(id) {
  ...
  if (demoMode) {
    const oldMin = s.quantiteMin || 0;
    const oldQty = s.quantite || 0;
    s.quantite = newQty;
    s.quantiteMin = parseInt(document.getElementById('es_min').value)||0;
    s.prix = parseFloat(document.getElementById('es_prix').value)||0;
    s.categorie = document.getElementById('es_cat').value;
    s.fournisseur = document.getElementById('es_four').value.trim();
    s.updatedAt = demoTimestamp();
    s.updatedBy = who();
...
async function deleteStock(id, nom) {
  ...
  if (demoMode) {
    cache.stock = cache.stock.filter(s => s._id !== id);
...
function openInventoryCount(id, nom) {
  ...
  openModal('📋',`Inventaire : ${nom}`,`
    <p style="color:var(--muted);margin-block-end:1rem;font-size:0.9rem">
    ...
    <button class="btn btn-gold" style="inline-size:100%;margin-block-start:0.5rem"
...
async function saveInventoryCount(id) {
  ...
  if (demoMode) {
    ...
    await logActivity('stock_inventory','📋',
      ...
      who());
    if (qty <= (s?.quantiteMin||0) && oldQty > (s?.quantiteMin||0)) {
      await pushNotif(
...
async function pushNotif(type, message, produit) {
  if (demoMode) {
    cache.notifications.unshift({
      _id:`demo-notif-${Date.now()}`,
      type, message, produit: produit||null,
      createdBy: who(), createdAt: demoTimestamp(), readBy: []
    });
    persistDemo();
    updateNotifBell();
    renderNotifPanel();
...
async function markAllRead() {
  ...
  if (demoMode) {
    unread.forEach(n => n.readBy = [...(n.readBy||[]), email]);
    persistDemo();
    updateNotifBell();
    renderNotifPanel();
...
async function logActivity(type, icon, description, user) {
  if (demoMode) {
    cache.activity.unshift({
      ...
      type, icon, description, user, timestamp: demoTimestamp()
    });
    cache.activity = cache.activity.slice(0, 30);
    persistDemo();
    renderActivityFeed();
    return;
  }
...
function renderReorderSuggestions() {
  ...
  el.innerHTML = low.map(s => {
    ...
    <div class="reorder-item">
      ...
      <button class="btn btn-teal btn-sm" style="margin-inline-start:0.5rem"
...
function renderConsumptionChart() {
  ...
  if (!window.Chart) {
    ...
    if (wrap) wrap.innerHTML = '<p style="color:var(--muted);font-size:0.9rem">Graphique indisponible hors connexion, les données restent consultables.</p>';
    return;
  }
...
function renderTopConsumed() {
  ...
  el.innerHTML = top.map(([n,q],i) => `
    <div style="margin-block-end:0.7rem">
      <div style="display:flex;justify-content:space-between;margin-block-end:0.3rem">
        <span style="font-size:0.85rem;font-weight:500">${i+1}. ${esc(n)}</span>
        <span style="font-size:0.78rem;color:var(--muted)">${q} vendu(s)</span>
      </div>
      <div style="block-size:4px;border-radius:2px;background:rgba(255,255,255,0.08);overflow:hidden">
        <div style="block-size:100%;border-radius:2px;inline-size:${Math.round(q/max*100)}%;
...
function openAddVente() {
  ...
  openModal('💰','Nouvelle Vente',`
    ...
    <div style="background:rgba(0,210,200,0.07);border:1px solid rgba(0,210,200,0.2);
      border-radius:10px;padding:0.8rem;text-align:center;margin-block-end:1rem">
    ...
    <button class="btn btn-teal" style="inline-size:100%" onclick="saveVente()">✓ Enregistrer la vente</button>
  `);
...
async function saveVente() {
  ...
  if (demoMode) {
    cache.ventes.unshift({
      _id:`demo-vente-${Date.now()}`,
      produit, quantite:qty, prix, total:qty*prix,
      date: demoTimestamp(), user: who()
    });
    if (stockId) {
      ...
      if (s) {
        ...
        s.updatedAt = demoTimestamp();
        s.updatedBy = who();
        if (newQty <= (s.quantiteMin||0) && oldQty > (s.quantiteMin||0)) {
          await pushNotif(
            newQty===0 ? 'stock_critical' : 'stock_low',
            newQty===0
...
async function deleteVente(id) {
  ...
  if (demoMode) {
    cache.ventes = cache.ventes.filter(v => v._id !== id);
    persistDemo();
    renderVentes();
    updateDashboard();
...
function loadEquipe() {
  ...
  grid.innerHTML = cache.equipe.map(e => `
    <div class="member-card" style="border-color:${e.actif?'rgba(16,185,129,0.2)':'var(--border)'}">
      ...
      <span class="badge ${e.actif?'badge-green':'badge-red'}" style="margin-block-end:1rem">
...
function openAddMembre() {
  openModal('👥','Nouveau Membre',`
    ...
    <button class="btn btn-teal" style="inline-size:100%;margin-block-start:0.5rem" onclick="saveMembre()">✓ Ajouter</button>
  `);
}
...
function renderServiceMode() {
  ...
  grid.innerHTML = items.map(s => {
    ...
    <div class="service-card" data-id="${s._id}" style="border-color:${border};${bgStyle}">
      <div style="flex:1;min-inline-size:0;overflow:hidden">
        <div style="font-size:0.68rem;color:var(--muted);margin-block-end:0.1rem">
          ${GAMMA_CATS[s.categorie]?.emoji||'📦'} ${esc(s.fournisseur||'')}
          ${isCrit ? '<span class="badge badge-red" style="margin-inline-start:0.4rem">Rupture</span>' : isLow ? '<span class="badge badge-amber" style="margin-inline-start:0.4rem">⚠ Faible</span>' : ''}
        </div>
      ...
      <div style="display:flex;align-items:center;gap:0.7rem;flex-shrink:0">
        ...
        <div style="min-inline-size:76px;text-align:center">
...
function renderCommande() {
  ...
  el.innerHTML = `<p style="color:var(--muted);font-size:0.85rem;margin-block-end:1.5rem">📅 ${today}</p>` +
  suppliers.map(sup => {
    ...
    <div class="commande-supplier">
      <div class="commande-sup-header">
        <div>
          ...
          <strong style="font-size:1.05rem;margin-inline-start:0.4rem">${sup.label}</strong>
          <span style="color:var(--muted);font-size:0.78rem;margin-inline-start:0.5rem">${sup.hint}</span>
          <span class="badge ${toOrder.length ? 'badge-red' : 'badge-green'}" style="margin-inline-start:0.6rem">
        ...
        <div style="display:flex;gap:0.5rem;flex-wrap:wrap">
          ...
          <button class="btn btn-ghost btn-sm" onclick="exportOrderPDF('${sup.id}','${esc(sup.label)}')">📄 PDF</button>
        </div>
      </div>
      ${toOrder.length === 0
        ...
        : toOrder.map(s => {
            ...
            <div class="commande-row">
              ...
              <span style="font-size:0.8rem;color:var(--muted);min-inline-size:70px">${esc(s.unite||'')}</span>
            </div>`;
          }).join('')
      }
      ${ok.length ? `
        <details style="border-block-start:1px solid var(--border)">
...
async function exportOrderPDF(supplierId, supplierLabel) {
  if (!window.jspdf) {
    showToast('Module PDF indisponible — utilisez Copier WhatsApp', 'warning');
    return;
  }
  ...
  const items = cache.stock.filter(s => s.fournisseur === supplierId);
  const lines = items
    .map(s => ({ nom: s.nom, qty: parseInt(document.getElementById('cmd_'+s._id)?.value||0, 10), unite: s.unite }))
    .filter(x => x.qty > 0);

  if (!lines.length) {
    showToast('Aucune quantité saisie pour ' + supplierLabel, 'info');
    return;
  }
  ...
  doc.setFontSize(22);
  doc.text(`BON DE COMMANDE : ${supplierLabel.toUpperCase()}`, 20, 20);
  
  doc.setFontSize(12);
  doc.setFont("helvetica", "normal");
  doc.text(`Date : ${new Date().toLocaleDateString('fr-FR')}`, 20, 30);
  doc.text(`Émis via Système GAMMA Stock`, 20, 36);
  ...
  lines.forEach(item => {
    ...
    doc.text(`${item.qty} ${item.unite || ''}`, 140, y);
    y += 8;
    if (y > 275) { doc.addPage(); y = 20; }
  });

  doc.save(`Commande_${supplierLabel}_${new Date().toISOString().slice(0,10)}.pdf`);
...
function openRecipe(id) {
  ...
  const costBadge = costMatches >= 2
    ? `<span class="badge badge-gold" style="margin-inline-start:0.5rem">💰 Coût ~${fMoney(estCost)}</span>`
    : '';

  openModal(CAT_EMOJI[r.category]||'🍽️', r.name, `
    <div style="display:flex;gap:0.5rem;flex-wrap:wrap;margin-block-end:0.5rem">
      <span class="badge" style="background:${CAT_BG[r.category]};color:${CAT_COLOR[r.category]}">
